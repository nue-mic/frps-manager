#!/usr/bin/env bash
# =============================================================================
# build-ipk.sh — 生成单个「all 架构」的 luci-app-frpsmgrd OpenWrt .ipk
#
#   该包不含 CPU 二进制，只装壳子（procd 脚本 + UCI 配置 + frpsmgrd-fetch）。
#   一个包到处可装，安装时由 postinst 调 frpsmgrd-fetch 按本机 CPU 联网下载
#   对应版本的二进制到 /usr/bin/frpsmgrd。
#
# 参数：
#   --version <x.y.z>   版本号（也可用环境变量 VERSION）。这是 frpsmgrd-fetch
...
# 随包文件
INITD_SRC="$ROOT_DIR/openwrt/files/etc/init.d/frpsmgrd"
CONFIG_SRC="$ROOT_DIR/openwrt/files/etc/config/frpsmgrd"
FETCH_SRC="$ROOT_DIR/openwrt/files/usr/sbin/frpsmgrd-fetch"
...
# LuCI web 壳子
LUA_CTRL="$ROOT_DIR/openwrt/luci-app-frpsmgrd/luasrc/controller/frpsmgr.lua"
LUA_VIEW="$ROOT_DIR/openwrt/luci-app-frpsmgrd/luasrc/view/frpsmgr/main.htm"
ACL_LUCI="$ROOT_DIR/openwrt/luci-app-frpsmgrd/root/usr/share/rpcd/acl.d/luci-app-frpsmgrd.json"
UCIDEF_LUCI="$ROOT_DIR/openwrt/luci-app-frpsmgrd/root/etc/uci-defaults/40_luci-frpsmgrd"
...
WORK="$(mktemp -d 2>/dev/null || mktemp -d -t frpsmgrd-ipk)"
cleanup() { [ -n "${WORK:-}" ] && rm -rf "$WORK"; }
trap cleanup EXIT INT TERM

# 生成随包 VERSION 文件（frpsmgrd-fetch 默认拉此版本）
...
OUTFILE="$OUT/luci-app-frpsmgrd_${VERSION}-${PKG_RELEASE}_all.ipk"
